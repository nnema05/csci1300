    ...
    while(!anyone_castle_reached) {
        ...
        for(int i = 0; i < numOfPlayers && !anyone_castle_reached; i++) {
            ...
            if(players[i].getSkipTurn() == true) {
                if(i == 0) { // then it was originally players 1 turn but now its players 2
                    player_index = i + 1;
            ...
            // Bool to keep track if player drew card or used candy (if they did, their turn is over)
            isDrawCard = false;
            isUseCard = false;
            drawCardAgain = false;
                ...
                /* USE CANDY*/
                    ...
                    if(players[player_index].getCandyAmount() == 0) {
                        ...
                        // Find player and opponent
                            // if numOfRounds%2 = 1, (num of rounds is a 1, 3, 5..) then we are on player 1 and opponent is player 2 or player i+1
                            // if numOfRounds%2 = 0, (num of rounds is a 2, 4, 6..) then we are on player 2 and opponent is player 1 or player i-1
                        if(player_index == 0) {
