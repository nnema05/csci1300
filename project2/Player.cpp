...
// get list of all candies that characters has with all of its information 
...
// Checks if a candy is off type immunity and returns the name of it 
...
// Checks if a candy is off type magical and returns the true or false of it 
...
// Checks if a candy is off type magical and returns the name of it 
...
// Find candy in a player and return that candy based on what they inputted
Candy Player::findCandy(string candy_name) {
    // see if you can find candy name in inventory
        // boolean 
    bool found = false;
    // for loop through the amount of candies in the player
    for(int i = 0; i < _candy_amount; i++) {
...
// Add candy to the players invnetory 
bool Player::addCandy(Candy candy) {
    ...
    if(_candy_amount < _MAX_CANDY_AMOUNT) {
        for(int i = 0; i < _MAX_CANDY_AMOUNT; i++) {
            // based on the inventory, if the inventory name is blank or the last one, then add it to the inventory  
...
// removes candy from the player based on the string name that is passed in
bool Player::removeCandy(string candy_name) {
    ...
    for(int i = 0; i < _candy_amount && !removed; i++) {
        ...
        // if candy name is found and removed is not true
        if(_inventory[i].name == candy_name && !removed) {
            removed = true; // set boolean equal to true 
...
bool Player::substituteCandy(Candy candy_to_substitute, Candy candy_to_buy) {
    ...
    // for loop through invnetory and find the spot where the candy name equals the name of the candy to substitue
    for(int i = 0; i < _candy_amount && !replaced; i++) {
        if(_inventory[i].name == candy_to_substitute.name && !replaced) { 
            replaced = true; 
            // replace the candy name 
            _inventory[i] = candy_to_buy; // then replace the candy to substitute with candy to buy
