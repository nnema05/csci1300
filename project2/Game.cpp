...
// Move the player based on the card
bool Game::moveThePlayer(Card card, Board &board, int player_number, Player players[], bool &drawCardAgain) {
    ...
    int move_amount = 0; // create a move amount and tile to move to 
    ...
    cout << "You drew the card color " << card._card_color << endl;
    ...
    /* FOR TESTING */ 
    // tile_to_move_to = 10;
    int movedfromboard = board.movePlayer(tile_to_move_to, player_number); // move the player board, changing its poition using the board move player function
    /* FOR TESTING */
    ...
    if(drawCardAgain == true) {
        drawCardAgain = false;
        board.setPlayersOnBoardStatus("draw card again", player_number); // set status to draw card again 
        return true;
    }
    ...
    if(player_number == 1) { // we are on players 1 turn, that means player 1 moved to where player 2 is originally 
        if(board.getPlayersVectorOnBoard().at(player_number - 1)._position_player_on_board == board.getPlayersVectorOnBoard().at(player_number)._position_player_on_board) {
            board.setPlayersOnBoardStatus("same tile", player_number); // set status as same tile
            // cout << " status " << board.getPlayersVectorOnBoard().at(player_number - 1)._status_player_on_board << endl;
            ...
            if(robbers_repel.name == "") {
                ...
                // IF THE PLAYER DOES HAVE ROBBERS REPEL
    ...
    /* SPECIAL TILE*/
    if(board.getTile(tile_to_move_to).getTileType() == "special tile") { // IF THE TILE TYPE THAT THE PLAYER IS MOVING TO  IS SPECIAL TYPE 
        ...
        if(r == 1) {
            ...
            if(position == 0) { // if old position was 0, set moved back to 0 as true
            ...
            board.movePlayer(tile_to_move_to, player_number); // move the player to the new position 
            string candyToRemove = players[player_number - 1].isImmunityCandy();
            // cout << candyToRemove << endl;
            if(candyToRemove != "") { // if the candy to remove is not blank (exists), then remove it
                // cout << "hi" << endl;
        ...
        // IF the player moves again and happens to land on the same tile - run all same time code again
        if(player_number == 1) { // we are on players 1 turn, that means player 1 moved to where player 2 is originally 
            if(board.getPlayersVectorOnBoard().at(player_number - 1)._position_player_on_board == board.getPlayersVectorOnBoard().at(player_number)._position_player_on_board) {
                // both players are on same tile
                // cout << "same tile" << endl;
    ...
    // set status based on what tile the player is on!!
    /* REGULAR TILE */
    ...
    if(board.getTile(tile_to_move_to).getTileType() == "calamity regular tile") {
        ...
        // Depending on what random number is, run a certain type of calamities (these are probablities!!)
        if(r < 6) {
            // Candy bandits
...
Card Game::drawACard(char correct_choice) {
    ...
    r = rand() % 9;
    // cout << "r " << r << endl;
    // Create a string of card colors
    ...
    // based on what random number is, set the card.color is struct to a certian card colo
    if (correct_choice == 'D') {
        if(r == 0 || r == 3) {
...
vector<Character> Game::readCharactersFile(Candy candy_array[], int current_size) {
    ...
    if(file.is_open())
    {
        ...
        while (!file.eof()) {
            ...
            // read all candies
            getline(ss, candy_line); // read whole list of candies 
            // cout << candy_line << endl;
            stringstream candy_string(candy_line);  // make candies a string stream
            // Divide up the candies based on comments!
            while(getline(candy_string, one_candy, ',')) {
                ...
                for(int i = 0; i < current_size; i++) {
                    ...
                    if(one_candy == candy_array[i].name) { // create into a struct that exists in the full candy array from the file
...
/* Remove characters from list of characters if player selects them */
...
void Game::visitCandyStore(Player &player, CandyStore &candy_store, string candy_to_buy, Candy candy_to_buy_sturct, Candy candies[], int &candy_current_size) {
    cout << "Here is the list of candies in the store: " << endl;
            ...
            // create the string of candy to buy into a struct!
            ...
            // if it fails or if the cnady to buy doesnt exist in the list of three random candies, add again!
            ...
            else { // Otherwise, just add the candy to the player normally and remove it from store
...
bool Game::rockPaperScissors(Player[]) {
    ...
    cout << "Player: Enter r, p, or s" << endl; // player enters a rock paper scissors 
    ...
    // Generate random number between 1-3 so computer can generate a random choice
    srand((unsigned)time(NULL));
    int r = 0;
    r = rand() % 3 + 1;
    if(r == 1) {
    ...
    cout << "Your choice was " << choice1 << ". And the computer's choice " << computer_choice << endl;
...
/* RIDDLES, call function when riddles are called */
