...
/* Riddle struct to track question and answer */
struct Riddle {
    string _question;
    string _answer;
};

/* Card struct is card color and is double */
struct Card {
    string _card_color;
    bool _isDouble;
};

/* Struct player on board to track the each players on position on the board and track the stamina and status  */
...
public:
    ...
    /* setters and getters for players on the board */
